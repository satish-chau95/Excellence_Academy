...
const Chatbot = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/v1/chat/ask`, {
        message: input,
      });
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const userMessage = { sender: "user", text: "Sent an image", image: reader.result };
        setMessages((prev) => [...prev, userMessage]);
      };
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.button
                  ...
                  className={`text-white rounded-full p-2 transition-colors ${
                    input.trim() ? "bg-orange-500 hover:bg-orange-600" : "bg-gray-400 cursor-not-allowed"
                  }`}
                >
                  <Send size={20} />
                </motion.button>
